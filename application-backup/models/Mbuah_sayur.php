...
{
	public function getAll($limit = 25, $not_in = "Basis Data")
	{
		if (!empty($not_in)) {
			$this->db->where_not_in("kategori", $not_in);
		}
		return $this->db->order_by("nama", "ASC")->get("ensiklopedia_data", $limit)->result();
	}
	...
	{
		return $this->db->where("kategori", "Algoritma dan Kompleksitas")->order_by("nama", "ASC")->get("ensiklopedia_data")->result();
	}

	public function getSayur()
	{
		return $this->db->where("kategori", "Basis Data")->order_by("created_at", "DESC")->get("ensiklopedia_data")->result();
	}
