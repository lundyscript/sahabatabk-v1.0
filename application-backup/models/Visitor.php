...
class Visitor extends CI_Model
{
	...
	function insert_viewer_perpage($data_id)
	{
		// Check bila sebelumnya data pengunjung sudah terrekam
		if (!isset($_COOKIE[$data_id])) {
			$ip = $this->ip_user();
			$browser = $this->browser_user();
			$os = $this->os_user();
			// Masa akan direkam kembali
			...
			$duration = time() + 60 * 60 * 24;
			// simpan kedalam cookie browser
			setcookie($data_id, $browser, $duration);
			// current time
			$dateTime = date('Y-m-d H:i:s');
			$where = array(
				"data_id" => $data_id,
			);
			$cek = $this->db->select("data_id")->where($where)->get("ensiklopedia_data")->row();
			if (!empty($cek)) {
				$this->db->query("UPDATE ensiklopedia_data SET total = (total+1) WHERE data_id='$data_id'");
			} else {
				$params = array("data_id" => $data_id, "total" => 1);

				$this->db->insert("ensiklopedia_data", $params);
			}
		}
	}
}
