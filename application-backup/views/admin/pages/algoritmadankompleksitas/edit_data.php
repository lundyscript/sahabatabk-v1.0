...
<form action="<?= base_url("admin/algoritmadankompleksitas/update_post") ?>" method="post" enctype="multipart/form-data">
	<div class="modal-body">
		<div class="row">
			<div class="col-md-12">
				<div class="form-group">
					<label for="">Judul Bab</label>
					<input type="text" name="jenis" value="<?= $content->jenis ?>" class="form-control" autofocus="autofocus" required>
				</div>
				<div class="form-group">
					<label for="">Judul Sub Bab</label>
					<input type="hidden" name="data_id" value="<?= $content->data_id ?>" required>
					<input type="text" name="nama" value="<?= $content->nama ?>" class="form-control" required>
				</div>
				<div class="form-group">
					<label for="">Deskripsi Mata Kuliah</label>
					<textarea name="deskripsi" id="" class="form-control" rows="8"><?= $content->deskripsi ?></textarea>
				</div>
				<div class="form-group">
