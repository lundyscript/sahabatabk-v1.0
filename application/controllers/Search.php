...
class Search extends CI_Controller
{
	...
	public function index()
	{
		...
		$removal = $stopwordRemoverFactory->createStopWordRemover();
		$time_start = microtime(true);
		$kataDasar = $stemmer->stem($query);
		$dasar = $removal->remove($kataDasar);
		$this->db->truncate("ensiklopedia_temporary");
		$this->db->truncate("ensiklopedia_rank");
		$text = implode(" ", array_unique(explode(" ", $dasar)));
		$this->vsm($text);
		$avg = $this->db->query("SELECT avg(cosine) as avg from ensiklopedia_rank")->row()->avg;
//		$avg = $cos;
		...
		if ($cek == 0) {
			...
			ensiklopedia_data.created_at")->from("ensiklopedia_rank")
				...
				->order_by("ensiklopedia_rank.cosine", "DESC")->group_by("ensiklopedia_rank.id_master")->get();
			$proses = substr((microtime(true) - $time_start), 0, 4);
	...
	public function detail($data_id = "")
	{
		...
		// $this->visit->insert_viewer_perpage($data_id);
